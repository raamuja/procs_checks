...
#                    For PRIVATE repos over HTTPS, set GIT_USERNAME and
#                    GIT_TOKEN (a Personal Access Token) in db.conf - the
#                    script authenticates via a throwaway credential helper
#                    so the token never appears in the URL, shell history,
#                    or `ps` output. SSH URLs use your normal ssh-agent/keys.
...
if is_git_url "${ARG_GIT_REPO}"; then
  ...
  # Never let git sit waiting for an interactive username/password prompt -
  # this is the #1 cause of a clone that appears to "hang" or "time out"
  # against a private repo. Fail fast with a clear error instead.
  export GIT_TERMINAL_PROMPT=0

  # If HTTPS credentials are configured (GIT_USERNAME + GIT_TOKEN in db.conf),
  # supply them via a throwaway askpass helper. This keeps the token out of
  # the URL (so it never ends up in .git/config, shell history, or `ps`).
  if [[ -n "${GIT_USERNAME:-}" && -n "${GIT_TOKEN:-}" ]]; then
    ASKPASS_SCRIPT="${WORKDIR}/git-askpass.sh"
    cat > "${ASKPASS_SCRIPT}" <<-EOASK
	#!/bin/sh
	case "\$1" in
