...
for NAME in ${ALL_NAMES}; do
  ...
  if [[ ${IN_SRC} -eq 1 && ${IN_PROD} -eq 0 ]]; then
    ...
    if [[ -z "${DIFF_HTML_TRIMMED}" ]]; then
      ...
      DIFF_PROC_ENTRIES+="\"$(js_escape_str "${NAME}")\":{\"git\":${GIT_JS},\"prod\":${PROD_JS}},
"
    fi
  fi
done
...
${DIFF_PROC_ENTRIES}
  ...
  function showDiffDetail(name){
    ...
    renderPane(document.getElementById('diffDetailGitPane'), entry.git);
    renderPane(document.getElementById('diffDetailProdPane'), entry.prod);
  }
