...
collapse_ase_star_expansion() {
  ...
  awk '
    ...
    function try_finalize(buf, ind,    lowbuf, fp) {
      lowbuf = tolower(buf)
      fp = match(lowbuf, /[[:space:]]from[[:space:]]/)
      if (fp == 0) return ""
      return ind "select *" substr(buf, fp)
    }
  ...
  if [[ "${SRC_HASH[${NAME}]}" == "${PROD_HASH[${NAME}]}" ]]; then
    ...
    DIFF_HTML="$(diff "${SRC_CLEAN}" "${PROD_CLEAN}" \
      | awk -v src_off="$((SRC_START-1))" -v prod_off="$((PROD_START-1))" '
          ...
          function normalize_prod_for_compare(   i, low, out, collecting, indent, acc, lowacc, fp, line) {
            ...
            for (i = 1; i <= nprod; i++) {
              ...
              if (!collecting && index(low, "adaptive server has expanded all") > 0 && index(low, "elements") > 0) {
                ...
                if (fp > 0) { out = out " " indent "select *" substr(acc, fp); continue }
                collecting = 1
                continue
              }
              if (collecting) {
                ...
                lowacc = tolower(acc)
                fp = match(lowacc, /[[:space:]]from[[:space:]]/)
                if (fp > 0) { out = out " " indent "select *" substr(acc, fp); collecting = 0; acc = "" }
                continue
              }
              out = out " " line
            }
