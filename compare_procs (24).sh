...
: "${OUTPUT_DIR:=./output}"
: "${OUTPUT_HTML:=}"
# Character set ASE's sp_showtext output actually comes back in. Most
# non-US ASE installs default to a single-byte set like ISO-8859-1
# rather than UTF-8, so an accented byte (e.g. the e in "Debut") is
# valid in that set but NOT valid UTF-8 - browsers then render it as
# the U+FFFD replacement glyph and, worse, it makes that line hash/diff
# as different from Git even when the character is identical. Check
# your server's actual default character set (ask your DBA team, or
# run sp_helpsort) and override here if it isn't ISO-8859-1.
: "${PROD_CHARSET:=ISO-8859-1}"
...
collapse_ase_star_expansion() {
  ...
  awk '
    ...
    function collapse_select_list(mid,    n, i, item, out, arr, in_run) {
      # $1 = the raw text between "select" and "from". Splits it on
      # top-level commas and reduces any run of plain qualified
      # table-column references (ASE always expands the original "*"
      # as "<table_or_#temptable>.<column>") back down to one "*",
      # while leaving any other item - a literal, an expression, an
      # "as" alias the developer wrote alongside the star - untouched
      # and in its original position. Only ever applied to PROD text.
      n = split(mid, arr, ",")
      out = ""
      in_run = 0
      for (i = 1; i <= n; i++) {
        ...
        if (item ~ /^#?[A-Za-z_][A-Za-z0-9_]*\.[A-Za-z_][A-Za-z0-9_]*$/) {
          if (!in_run) { out = out (out == "" ? "" : ", ") "*"; in_run = 1 }
        } else {
          out = out (out == "" ? "" : ", ") item
          in_run = 0
        }
    ...
    function try_finalize(buf, ind,    lowbuf, fp, sp, prefix, selend, mid) {
      lowbuf = tolower(buf)
      ...
      selend = sp + 6
      mid = substr(buf, selend, fp - selend)
      return ind prefix "select " collapse_select_list(mid) substr(buf, fp)
    }
    BEGIN { collecting = 0; acc = ""; indent = "" }
    {
      ...
      if (!collecting && index(low, "adaptive server has expanded all") > 0 && index(low, "elements") > 0) {
        # Preserve everything BEFORE the marker comment on this line, not
        # just leading whitespace - e.g. "else/* Adaptive Server has
        # expanded... */" or "if exists (select * ...)" style prefixes
        # must survive, or the reconstructed text silently loses that
        # code and no longer matches Git even when the actual "select *"
        # is a true match. cpos is the position of the comment start "/*".
        cpos = index(line, "/*")
        indent = (cpos > 0) ? substr(line, 1, cpos - 1) : leading_ws(line)
...
while IFS= read -r PNAME; do
  ...
  # Re-encode into UTF-8 up front so hashing, normalization, and the
  # HTML display (declared UTF-8) all work off consistent bytes instead
  # of PROD's raw single-byte-charset output. -c drops any byte that
  # still can't be mapped rather than aborting the whole extraction.
  if iconv -f "${PROD_CHARSET}" -t UTF-8 -c "${RAW_FILE}" > "${RAW_FILE}.utf8" 2>/dev/null; then
    mv "${RAW_FILE}.utf8" "${RAW_FILE}"
  else
    rm -f "${RAW_FILE}.utf8"
  fi
  ...
  if [[ ${IN_SRC} -eq 1 && ${IN_PROD} -eq 0 ]]; then
    ...
    DIFF_HTML="$(diff "${SRC_CLEAN}" "${PROD_CLEAN}" \
      | awk -v src_off="$((SRC_START-1))" -v prod_off="$((PROD_START-1))" '
          ...
          function collapse_select_list(mid,    n, i, item, out, arr, in_run) {
            # Mirrors collapse_select_list() in collapse_ase_star_expansion():
            # fold only the plain qualified table-column items (ASE own
            # expansion of the original "*") back down to one "*", keep
            # everything else (a literal, an alias, an expression the
            # developer wrote alongside the star) exactly as-is.
            n = split(mid, arr, ",")
            out = ""
            in_run = 0
            for (i = 1; i <= n; i++) {
          ...
          function normalize_prod_for_compare(   i, low, out, collecting, indent, acc, lowacc, fp, sp, prefix, line) {
            ...
            for (i = 1; i <= nprod; i++) {
              ...
              if (!collecting && index(low, "adaptive server has expanded all") > 0 && index(low, "elements") > 0) {
                marker_found = 1
                # Mirrors the same fix in collapse_ase_star_expansion():
                # keep any code sitting before the marker comment start
                # "/*" on this line (e.g. "else"), not just whitespace,
                # or this hunk can never reconstruct back to an exact
                # match against Git and gets shown as a false difference.
                cpos = index(line, "/*")
                indent = (cpos > 0) ? substr(line, 1, cpos - 1) : leading_ws(line)
                acc = find_after_marker(line, low)
                lowacc = tolower(acc)
                ...
                if (fp > 0) {
                  ...
                  selend = (sp > 0) ? sp + 6 : 1
                  mid = substr(acc, selend, fp - selend)
                  out = out " " indent prefix "select " collapse_select_list(mid) substr(acc, fp); continue
                }
                collecting = 1
                continue
              }
              if (collecting) {
                ...
                if (fp > 0) {
                  ...
                  selend = (sp > 0) ? sp + 6 : 1
                  mid = substr(acc, selend, fp - selend)
                  out = out " " indent prefix "select " collapse_select_list(mid) substr(acc, fp); collecting = 0; acc = ""
                }
                continue
              }
              out = out " " line
          ...
          function is_ase_star_expansion_only_hunk(   i, sconcat, prod_has_marker, reconstructed) {
            ...
            # nothing but (optional leading text plus) "select *" - or an
            # aliased/qualified star like "select audi2.*" or "select
            # t1.*", which ASE expands exactly the same way - with no
            ...
            if (prod_has_marker && sconcat ~ /select [a-z0-9_]*\.?\*[[:space:]]*$/) return 1
...
<script>
  function toggleDiff(name){
    ...
    if(!row) return;
    // Section 1 only ever shows/hides its own inline snapshot here. It
    // must NOT jump down to or drive section 1.1 - that view only updates
    // when the user explicitly picks a procedure from its own dropdown.
