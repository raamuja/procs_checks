...
normalize_file() {
  ...
  #
  # A standalone "go" line is an isql/client batch terminator, not part of
  # the actual stored procedure body - Sybase never persists it, so
  # sp_showtext extraction from PROD will never include it. Git .sql files
  # commonly DO include a trailing "go", which would otherwise show up as
  # a false difference against an identical PROD proc. Must run before the
  # whitespace-collapsing step below, while lines still exist to match against.
  #
  # Also bound the comparison to the actual procedure body: from the
  # "create [or replace] proc[edure] ..." line through the LAST standalone
  # "end" line in the file. Procedures often have nested begin/end blocks
  # (inside if/while), so we anchor on the *last* standalone "end" rather
  # than the first - for a well-formed proc that is always the outer
  # closing end. Anything outside that window (leading comments, trailing
  # blank lines, stray artifacts from either extraction path) is excluded
  # so it can never cause a false diff. If either boundary can't be found,
  # fall back to the whole file rather than risk truncating real content.
  sed -e 's#/\*.*\*/##g' "$1" \
    | sed -e 's/--.*$//' \
    ...
    | awk '
        ...
        tolower($0) ~ /^[[:space:]]*end[[:space:];]*$/ { endline = NR }
        END {
          s = (start > 0) ? start : 1
          e = (endline >= s) ? endline : last
