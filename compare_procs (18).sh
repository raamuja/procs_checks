  ...
  if [[ ${IN_SRC} -eq 1 && ${IN_PROD} -eq 0 ]]; then
    ...
    DIFF_HTML="$(diff "${SRC_CLEAN}" "${PROD_CLEAN}" \
      | awk -v src_off="$((SRC_START-1))" -v prod_off="$((PROD_START-1))" '
          ...
          function is_ase_star_expansion_only_hunk(   i, sconcat, prod_has_marker) {
            ...
            # compared byte-for-byte against Git side.
            if (norm_ws_lower(normalize_prod_for_compare()) == sconcat) return 1
...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .jump-controls{display:flex;align-items:center;gap:10px;margin-bottom:14px;flex-wrap:wrap;}
  .jump-controls label{font-size:12.5px;color:var(--muted);font-weight:600;}
  #diffProcSelect{padding:7px 10px;border:1px solid var(--line);border-radius:8px;font-size:13px;font-family:inherit;min-width:260px;max-width:420px;}
  #diffProcSelect:focus{outline:2px solid #fca5a5;outline-offset:1px;}
...
<script>
  ...
  // Jumps straight to a procedure's diff detail (used by the "Jump to a
  // differing procedure" dropdown in section 1) - force-opens it (never
  // toggles closed, unlike clicking the row) and scrolls it into view so
  // the user never has to scroll to find it.
  function jumpToDiff(name){
    if(!name) return;
    var detail = document.getElementById('diff-' + name);
    if(!detail) return;
    detail.style.display = 'table-row';
    var summary = document.getElementById('diffrow-' + name);
    (summary || detail).scrollIntoView({behavior:'smooth', block:'start'});
  }
...
${PROC_ENTRIES}
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    // Section 1's dropdown only ever lists procedures that actually
    // differ - built from PROC_DATA's own status, not the full list -
    // and picking one jumps straight to it instead of requiring scrolling.
    var diffSelect = document.getElementById('diffProcSelect');
    if (diffSelect){
      var diffNames = names.filter(function(n){ return PROC_DATA[n].status === 'DIFF'; });
...
<div class="wrap">
  ...
  <section>
    ...
    <div class="jump-controls">
      <label for="diffProcSelect">Jump to a differing procedure:</label>
      <select id="diffProcSelect">
