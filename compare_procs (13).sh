...
collapse_ase_star_expansion() {
  ...
  #
  # Handles the common case where the expanded "select ... from" lands on
  # a single line (the normal case, since ASE substitutes the columns
  # in-place). Rare multi-line expansions (extremely wide tables) are not
  # currently collapsed and would still show as a DIFF - flag it if seen.
  awk '
    BEGIN { collapse = 0 }
    {
      line = $0
      low  = tolower(line)
      if (index(low, "adaptive server has expanded all") > 0 && index(low, "elements") > 0) {
        collapse = 1
        next   # drop the marker comment itself - it is ASE noise, not source
      }
      if (collapse == 1) {
        if (match(low, /^[[:space:]]*select[[:space:]]+.+[[:space:]]from[[:space:]]/)) {
          sel_end  = index(low, "select") + length("select")
          from_pos = match(low, /[[:space:]]from[[:space:]]/)
          prefix = substr(line, 1, sel_end - 1)
          suffix = substr(line, from_pos)
          print prefix " * " suffix
          collapse = 0
          next
        }
...
for NAME in ${ALL_NAMES}; do
  ...
  if [[ "${SRC_HASH[${NAME}]}" == "${PROD_HASH[${NAME}]}" ]]; then
    ...
    extract_proc_body "${SRC_NOCR}" "${SRC_CLEAN}"
    extract_proc_body "${PROD_NOCR}" "${PROD_CLEAN}"
