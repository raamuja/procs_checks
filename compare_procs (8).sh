...
while IFS= read -r PNAME; do
  ...
  # NOTE: We deliberately use sp_helptext here instead of querying
  # syscomments.text directly. syscomments stores a proc's body split
  # across multiple rows in fixed-length chunks that do NOT respect word
  # boundaries, so a chunk cut can land mid-word (e.g. "#ADF_" / "QUERIES"
  # on two separate output lines). isql then prints each chunk-row on its
  # own line, and normalize_file's whitespace collapsing turns that
  # artificial line break into a space - silently splitting one token into
  # two and causing a false DIFF against an identical Git source.
  # sp_helptext reconstructs the text using real source line boundaries,
  ...
  # sp_helptext is itself a stored proc call, so isql automatically appends
  # a trailing "(return status = N)" line to the output of every single
  # invocation. That line is never part of the actual proc source (Git
  # never contains it), so without stripping it here, EVERY procedure would
  # hash as different from Git even when the content is identical.
